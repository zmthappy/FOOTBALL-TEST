...
const leagueList = ["日职联", "韩K联", "俄超", "意甲", "德甲", "英超", "英冠", "西甲",
    ...
    "巴甲", "挪超", "日职乙", "欧国联", "天皇杯", "意杯", "葡杯", "世亚预", "德国杯", "世南美预", "法国杯", "欧冠杯", "国际友谊", "日联杯", "欧联杯", "解放者杯", "欧洲杯", "美洲杯", "美职联", "欧协联"]
...
function getTimeRangeData(start = dayjs().format("YYYY-MM-DD"), end = dayjs().format("YYYY-MM-DD")) {
    ...
    // function getTimeRangeData(start = "2024-04-01", end = "2024-05-01") {
    // function getTimeRangeData(start = dayjs().add(-5, "day").format("YYYY-MM-DD"), end = dayjs().add(-3, "day").format("YYYY-MM-DD")) {
...
async function ttyqFootBallData(toDayTime, callBack, selfData) {
    ...
    let data = await getData(`https://www.ttyingqiu.com/static/no_cache/league/zc/jsbf/ttyq2020/sfc/sfc14c_${toDayTime}.json?v=${toDay.getTime()}`, {});
    ...
    let jcData = await getData(`https://www.ttyingqiu.com/static/no_cache/league/zc/jsbf/ttyq2020/sfc/${toDayTime}/oz_401_7.json??v=${toDay.getTime()}`, {})
    if (data == 404 || !data) {
        callBack(null, 404);
        return
    };
