...
//     return new Promise((resolve, reject) => {
//         request.Get(url, data, "utf-8").then((res) => {
//             resolve(res);
//         })
//     })
// }
...
function getTimeRangeData(start = dayjs().format("YYYY-MM-DD"), end = dayjs().format("YYYY-MM-DD")) {
    // function getTimeRangeData(start = dayjs().add(-2, "day").format("YYYY-MM-DD"), end = dayjs().add(-1, "day").format("YYYY-MM-DD")) {
...
const leagueList = ["日职联", "韩K联", "俄超", "意甲", "德甲", "英超", "英冠", "西甲",
    ...
    "巴甲", "挪超", "日职乙", "意杯", "葡杯", "德国杯", "法国杯", "欧冠杯", "日联杯", "欧联杯", "解放者杯", "欧协联"]
...
// 降板马
const subLeagueTeam = ["格罗宁根", "坎布尔", "埃门", "昂热", "阿雅克肖",
    "特鲁瓦", "欧塞尔", "沙尔克04", "柏林赫塔", "莱斯特城", "利兹联", "南安普顿", "横滨FC"];
// 国家队比赛
const countryMatch = ["国际友谊", "世亚预", "欧洲杯", "亚洲杯", "非洲杯", "世界杯", "美职业"]
...
async function ttyqFootBallData(toDayTime, callBack, selfData) {
    ...
    let jcData = await getData(`https://www.ttyingqiu.com/static/no_cache/league/zc/jsbf/ttyq2020/bd/${toDayTime}/oz_405_8.json?v=${toDay.getTime()}`, {})
    if (data == 404 || !data) {
        callBack(null, 404);
        return
    };
...
async function getAsia(item, obj, callBack) {
    ...
    fs.writeFileSync('./src/data/newBDDataFootball.json', JSON.stringify(computeList), "UTF-8");
}
